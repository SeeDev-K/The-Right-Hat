...
const defaultRows: AcademyItem[] = [
  { id: 'default-1', title: 'Cybersecurity Foundations', status: 'draft', slug: 'cybersecurity-foundations', summary: 'Core learning path for cyber fundamentals.' },
  { id: 'default-2', title: 'Incident Response Essentials', status: 'draft', slug: 'incident-response-essentials', summary: 'Operational response and escalation basics.' },
  { id: 'default-3', title: 'Threat Intelligence Lab', status: 'published', slug: 'threat-intelligence-lab', summary: 'Structured analysis and reporting practice.' },
]
...
export function SimpleAcademyBoard() {
  ...
  useEffect(() => {
    async function loadAcademyItems() {
      ...
      if (error || !data || data.length === 0) { setItems(defaultRows); setUsingFallback(true) } else { setItems(data as AcademyItem[]); setUsingFallback(false) }
      setLoading(false)
    }
    loadAcademyItems()
  }, [])
  ...
  async function addAcademyItem(event: FormEvent<HTMLFormElement>) {
    ...
    const { data, error } = await supabase
      .from('content_items')
      .insert({ title: cleanTitle, slug: slugify(cleanTitle), summary: cleanSummary || null, kind: 'academy', status: 'draft' })
      .select('id,title,status,slug,summary,created_at')
      .single()
  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-950 p-6 text-slate-100 shadow-2xl shadow-slate-950/30">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-xs font-black uppercase tracking-[.18em] text-amber-300">Academy CMS</p><h2 className="mt-2 text-3xl font-black text-white">Academy board</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-400">Draft and review academy content from Supabase.</p></div>
      ...
      <form onSubmit={addAcademyItem} className="mt-6 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
        <input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="New academy item title" className="border-slate-700 bg-slate-900 text-white placeholder:text-slate-500 focus:border-amber-300 focus:shadow-none" />
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-slate-800">
        ...
        {loading ? <div className="px-4 py-5 text-sm text-slate-400">Loading academy items...</div> : <div className="divide-y divide-slate-800">{items.map((item) => <article key={item.id} className="grid items-center gap-4 px-4 py-4 md:grid-cols-[1fr_160px_120px_360px]"><div><h3 className="font-bold text-white">{item.title}</h3>{item.summary && <p className="mt-1 text-xs text-slate-500">{item.summary}</p>}</div><span className="truncate font-mono text-xs text-slate-500">{item.slug || slugify(item.title)}</span><span className="w-fit rounded-full border border-slate-700 px-3 py-1 text-xs font-black uppercase tracking-[.14em] text-amber-200">{item.status || 'draft'}</span><div className="flex flex-wrap gap-2">{statusActions.map((action) => { const isActive = item.status === action.value; return <button key={action.value} type="button" disabled={updatingId === item.id || isActive} onClick={() => updateAcademyStatus(item.id, action.value)} className={['rounded-full border px-3 py-1.5 text-xs font-black uppercase tracking-[.12em] transition disabled:cursor-not-allowed', isActive ? 'border-amber-300 bg-amber-300 text-slate-950' : 'border-slate-700 bg-slate-900 text-slate-300 hover:border-amber-300 hover:text-amber-200', updatingId === item.id && !isActive ? 'opacity-60' : ''].join(' ')}>{action.label}</button> })}</div></article>)}</div>}
      </div>
    </section>
  )
}
