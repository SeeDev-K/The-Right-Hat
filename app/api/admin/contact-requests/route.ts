...
export async function GET(request: Request) {
  try {
    ...
    const response = await fetch(`${url}/rest/v1/contact_requests?select=*&order=created_at.desc&limit=100`, { headers: { apikey: serviceRoleKey, Authorization: `Bearer ${serviceRoleKey}` }, cache: 'no-store' })
