...
export default function StaffAccessPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(false)
    if (signInError) { setFailed((x) => x + 1); setError('Access denied.'); return }
        ...
        <section className="flex items-center justify-center px-6 py-12 lg:px-12">
          <div className="w-full max-w-xl">
            ...
            <form onSubmit={onSubmit} className="mt-8 grid gap-5 rounded-[32px] border border-white/10 bg-white/[.06] p-8 shadow-2xl shadow-blue-950/30 backdrop-blur-xl">
              ...
              <button disabled={loading} className="rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-400 px-5 py-4 font-black text-white shadow-lg shadow-blue-600/25" type="submit">{loading ? 'Verifying...' : 'Enter control center'}</button>
              <p className="text-xs font-bold text-slate-400">Private staff access · Role and module permissions checked after login</p>
