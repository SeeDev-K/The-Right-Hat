...
const sections = [
  ...
  ['Audit logs', 'Administrative events and sensitive activity review.'],
]
  ...
  return (
    <main className="min-h-screen bg-[#070b12] p-5 text-white lg:p-10">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8"><AdminModuleGuard module="settings"><div className="grid gap-4 md:grid-cols-2">{sections.map(([title, text]) => <article key={title} className="rounded-[28px] border border-white/10 bg-white/[.05] p-6"><h2 className="text-2xl font-black">{title}</h2><p className="mt-3 text-slate-400">{text}</p><button className="mt-6 rounded-2xl border border-white/10 px-4 py-3 text-sm font-black text-slate-300">Configure</button></article>)}</div></AdminModuleGuard></div>
      </div>
    </main>
  )
}
