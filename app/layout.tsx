      ...
      <body>
          ...
          <div className="container flex h-20 items-center justify-between">
            <Link href="/" className="flex items-center gap-3 font-black tracking-tight">
              ...
              <span className="leading-tight">
                <span className="block text-lg">The Right Hat</span>
                <span className="block text-xs font-bold uppercase tracking-[.22em] text-slate-400">Cyber · Academy · Intel</span>
              </span>
            </Link>
            <nav className="hidden items-center gap-6 text-sm font-bold text-slate-300 md:flex">
              {nav.map(([label, href]) => (
                <Link key={href} href={href} className="hover:text-[var(--gold-soft)]">{label}</Link>
              ))}
            </nav>
        ...
        <footer className="border-t border-[var(--border)] py-10 text-sm text-slate-400">
          <div className="container grid gap-6 md:grid-cols-[1fr_auto] md:items-center">
            <div>
              <strong className="block text-base text-white">TRH — The Right Hat</strong>
              <span>Premium cybersecurity, training, intelligence and community operations.</span>
            </div>
